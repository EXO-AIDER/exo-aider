...
struct MySensorbandTask : TaskInterface {
  ...
  bool get_low_frequency_samples(vector<float> &low_frequency_samples, bool sending_signals){
        ...
        low_frequency_samples.push_back(_IMU1.getAccelY_mss());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU1.getAccelZ_mss());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU1.getGyroX_rads());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU1.getGyroY_rads());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU1.getGyroZ_rads());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU2.getAccelX_mss());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU2.getAccelY_mss());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU2.getAccelZ_mss());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU2.getGyroX_rads());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU2.getGyroY_rads());                                                                  // Reinterprets IMU data float to uint8_t data 
        low_frequency_samples.push_back(_IMU2.getGyroZ_rads());                                                                  // Reinterprets IMU data float to uint8_t data 
      ...
      _ADC.noOpDaisy();
       /*EMG*/
        ...
        for (size_t i = 0; i < EMG.size(); i++){   
            high_frequency_samples.push_back(EMG[i]);                                                                                 // Reinterprets IMU data float to uint8_t data 
        }
    // What goes here:
    // * EMG signal sampling
    return true;
